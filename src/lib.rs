...
pub mod violation {
    ...
    pub const ALL               : ViolationMask = TRAVEL_TIME;
    pub const RAW_COORD_REQUIRED: ViolationMask = TRAVEL_TIME;
}
...
pub enum StickType { Analog, Digital }
...
pub struct PlayerViolations {
    ...
    /// Whether the lstick is analog or digital.
...
pub fn check_game(game: &Game) -> ViolationResult {
    ...
    let mut lstick_coords = Vec::new();
    let mut rim_coords = std::collections::HashSet::<(i8, i8)>::with_capacity(4096);
    
    for ply in 0..4 {
        let Some(ref frames) = game.frames[ply] else { continue; };
        let vio = &mut res.players[ply];
        
        // push clamped coords to buffer
        lstick_coords.clear();
        lstick_coords.reserve(frames.len());
        for f in frames {
            lstick_coords.push(f.left_stick_coords_raw.clamped());
        }
        
        // determine digital/analog lstick
        vio.lstick_type = stick_type(&mut rim_coords, &lstick_coords);
        
        // skip some checks if analog
        if vio.lstick_type == StickType::Digital {
            if res.checked & violation::TRAVEL_TIME != 0 {
                vio.travel_time_hit_rate = travel_time_hit_rate(&lstick_coords);
...
fn stick_type(
    ...
    rim_coords.clear();
    const RIM: isize = (VectorI8::MAX as isize - 1).pow(2);
    
    for coord in coords.iter().copied() {
        let x_sq = (coord.x as isize).pow(2);
        let y_sq = (coord.y as isize).pow(2);
        if x_sq + y_sq >= RIM {
            rim_coords.insert((coord.x, coord.y));
        }
    }
    
    const RIM_COORD_MAX: f32 = 432.0;
    let mut rim_proportion = rim_coords.len() as f32 / RIM_COORD_MAX;
    
    const THREE_MINUTES: usize = 10800;
    if coords_len < THREE_MINUTES {
        let boost = (THREE_MINUTES - coords_len) as f32 / THREE_MINUTES as f32 + 1.0;
        rim_proportion *= boost;
    }
    
    if rim_proportion < 0.5 {
        StickType::Digital
    } else {
        StickType::Analog
    }
